...
<body>
	<div id="main">
		...
		try{
			...
			PreparedStatement pre=con.prepareStatement("select count(*) from  libaddfaculty where facultyid=?");
			pre.setString(1, request.getParameter("id"));
			ResultSet res=pre.executeQuery();
			if(res.next()){
				int i=res.getInt(1);
				if(i==1){
					pre=con.prepareStatement("delete from libaddfaculty where facultyid=?");
					pre.setString(1, request.getParameter("id"));
					i=pre.executeUpdate();
					if(i==1){
						con.commit();
