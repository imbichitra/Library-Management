...
<body>
	<div id="main">
		...
		<article>
			<header>
				...
				<form action="RemoveStudent.jsp">
					<select style="width: 100px; height:25px" name="id" class="select">
					...
					<%
						try{
							...
							ResultSet res=pre.executeQuery();
							 while(res.next()){
						%>
							<option value=""><%=res.getString("REGNO") %></option>
						 <%}
		...
		try{
			Class.forName("oracle.jdbc.driver.OracleDriver");
			Connection con= DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe", "system", "manager");
			PreparedStatement pre=con.prepareStatement("select count(*) from  libaddstudent where regno=?");
			pre.setString(1, request.getParameter("id"));
			ResultSet res=pre.executeQuery();
			if(res.next()){
				int i=res.getInt(1);
				if(i==1){
					pre=con.prepareStatement("delete from libaddstudent where regno=?");
					pre.setString(1, request.getParameter("id"));
					i=pre.executeUpdate();
					if(i==1){
						con.commit();
						out.print("<label style='color:red'><h4>"+"delete"+"</h4></label>");
					}
				}
				else{
